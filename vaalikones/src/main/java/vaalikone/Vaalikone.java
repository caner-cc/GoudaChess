...
public class Vaalikone extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        kysymysSize = kaikkiKysymykset.size(); 
        
        List<Integer> kysymyksetID = new ArrayList<Integer>();
        for(Kysymykset k : kaikkiKysymykset) {
        	kysymyksetID.add(k.getKysymysId());
        }
        ...
        if (strFunc == null) {
            ...
            if (strKysymys_id == null) {
                ...
                try {
                	kysymys_id = kysymyksetID.get(kysymyksetID.indexOf(kysymys_id) + 1);
                }
            ...
            if (kysymyksetID.contains(kysymys_id)) {
                ...
                for (int i = 0; i < ehdokasList.size(); i++) {
                    ...
                    for (Vastaukset eVastaus : vastausList) {
                        ...
                        usr.addPisteet(ehdokasList.indexOf(ehdokas_id), ehdokas_id, pisteet);
                    }
        ...
        if ("haeEhdokas".equals(strFunc)) {
            ...
            List<Tuple<Integer, Integer>> sortedEhdokkaat = usr.haeParhaatEhdokkaat();

            //hae määritetyn ehdokkaan tiedot
            Query q = em.createQuery("SELECT e FROM Ehdokkaat e WHERE e.ehdokasId=?1");
            q.setParameter(1, sortedEhdokkaat.get(jarjestysnumero).ehdokasId);
            ...
            request.setAttribute("ehdokasList", ehdokasList);
            ...
            request.setAttribute("pisteet", sortedEhdokkaat.get(jarjestysnumero).pisteet);
